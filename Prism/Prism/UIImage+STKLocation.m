...
+ (void)LocationCoordinateFromImageInfo:(NSDictionary *)imageInfo completion:(void(^)(NSError *, CLLocationCoordinate2D))completion
{
    ...
    __block CLLocationCoordinate2D locationCoordinate = locationCoordinate = CLLocationCoordinate2DMake(0, 0);
    [library assetForURL:[imageInfo objectForKey:UIImagePickerControllerReferenceURL] resultBlock:^(ALAsset *asset) {
        ...
        if (length != 0){
            ...
            if (gps) {
                ...
                err = [NSError errorWithDomain:@"STKLocation" code:5001 userInfo:nil];
            }
        } else {
            err = [NSError errorWithDomain:@"STKLocation" code:5002 userInfo:nil];
        }
        completion(err, locationCoordinate);
